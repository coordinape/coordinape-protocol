...
def test_root_upload(ape_reg, ape_fee, ape_distro, ape_router_registry_beacon, ape_factory_registry_beacon, big_usdc, usdc, ApeVaultWrapperImplementation, minter, interface):
    ...
    with reverts('ApeDistributor: Sender not approved'):
    ...
    assert ape_distro.epochRoots(ape_vault, circle, token, 0) == root
...
def test_claiming(ape_reg, ape_fee, ape_distro, ape_router_registry_beacon, ape_factory_registry_beacon, big_usdc, usdc, ApeVaultWrapperImplementation, minter, interface):
    ...
    with reverts('Wrong proof'):
        ape_distro.claim(ape_vault, circle, usdc_vault, 0, add_0_data['index'], adds[1], add_0_data['amount'], False, add_0_data['proof'], {'from':user})
    ape_distro.claim(ape_vault, circle, usdc_vault, 0, add_0_data['index'], adds[0], add_0_data['amount'], False, add_0_data['proof'], {'from':user})
    ...
    ape_distro.claim(ape_vault, circle, usdc_vault, 0, add_1_data['index'], adds[1], add_1_data['amount'], True, add_1_data['proof'], {'from':adds[1]})
    assert usdc.balanceOf(adds[1]) - pre  >= 3_000_000_000
    assert ape_distro.checkpoints(ape_vault, circle, usdc_vault, adds[1]) == 3_000_000_000
...
def test_claiming_many(ape_reg, ape_fee, ape_distro, ape_router_registry_beacon, ape_factory_registry_beacon, big_usdc, usdc, ApeVaultWrapperImplementation, minter, interface):
    ...
    claim_data = []
    for i in range(5):
        claim_data.append((ape_vault.address, circles[i], tokens[i], epochs_[i], indexes[i], adds[i], checkpoints[i], redeems[i], proofs[4 - i]))
    with reverts('Wrong proof'):
        ape_distro.claimMany(claim_data, {'from':user})
    ...
    claim_data = []
    for i in range(5):
        claim_data.append((ape_vault.address, circles[i], tokens[i], epochs_[i], indexes[i], adds[i], checkpoints[i], redeems[i], proofs[i]))
    ape_distro.claimMany(claim_data, {'from':user})
    with reverts('Claimed already'):
        ape_distro.claimMany(claim_data, {'from':user})
    assert ape_distro.checkpoints(ape_vault, circle, usdc_vault, adds[1]) == 3_000_000_000
...
def test_attack(ape_reg, ape_fee, ape_distro, ape_router_registry_beacon, ape_factory_registry_beacon, big_usdc, usdc, ApeVaultWrapperImplementation, minter, interface, chain):
    ...
    with reverts("Can't claim more than circle has to give"):
        ape_distro.claim(rogue_ape_vault, rogue_circle, token, epoch, index, claimer, rogue_grant, False, proof, {'from':accounts[1]})

def test_attack_2(ape_reg, ape_fee, ape_distro, ape_router_registry_beacon, ape_factory_registry_beacon, big_usdc, usdc, ApeVaultWrapperImplementation, minter, interface, chain):
    setup_protocol(ape_reg, ape_fee, ape_distro, ape_router_registry_beacon, ape_factory_registry_beacon, minter)
    user = accounts[0]
    deposit = 11_000_000_000_000
    usdc.transfer(user, deposit, {'from':big_usdc})
    ...
    with reverts("Can't claim more than circle has to give"):
        ape_distro.claim(rogue_ape_vault, rogue_circle, token, epoch, index, claimer, rogue_grant, False, proof, {'from':accounts[1]})
    assert usdc_vault.balanceOf(claimer) == 0